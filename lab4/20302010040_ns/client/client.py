...
def ns_authentication(sock, server_name):
    ...
    server_public_key = ns.get_public_key(pks_address, host_name, NAME, rsa.import_key(private_key))

    # A -- {N_A, A}(K_PB) --> B
    n_a = ns.generate_nonce()
    # print(server_public_key)
    request = rsa.big_encrypt(rsa.import_key(server_public_key), f"{n_a},{NAME}")
    # print(request)
    ...
    request2 = rsa.big_encrypt(rsa.import_key(server_public_key), f"{aes_key.decode()},{n_b_received}")
