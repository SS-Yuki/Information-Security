...
def vigenere_encrypt(plaintext, key, alphabet):
    ...
    ciphertext = ""
    key_len = len(key)
    prepared_plaintext = prepare_string(plaintext, alphabet)

    i = 0
    for c in prepared_plaintext:
        key_c = key[i % key_len]

        c_index = c2i(c, alphabet)
        key_c_index = c2i(key_c,alphabet)

        encrypted_index = (c_index + key_c_index) % len(alphabet)
        encrypted_c = i2c(encrypted_index, alphabet)
...
def vigenere_decrypt(ciphertext, key, alphabet):
    ...
    plaintext = ""
    key_len = len(key)
    prepared_ciphertext = prepare_string(ciphertext, alphabet)

    i = 0
    for c in prepared_ciphertext:
        key_c = key[i % key_len]
        ...
        decrypted_c = i2c(decrypted_index, alphabet)
